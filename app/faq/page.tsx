...
const faqs: FAQ[] = [
  ...
  },
  {
    ...
    a: "Primara currently serves South Florida (Miami-Dade, Broward, Palm Beach, Martin, and Indian River Counties), the Tampa Bay area (Hillsborough, Pinellas, and Polk Counties), Orlando Metro (Orange, Osceola, and Seminole Counties), and the Jacksonville area (Duval, St. Johns, Clay, and Nassau Counties). Our market coverage is a deliberate choice, not a capacity limitation. Local search dominance requires the kind of market-specific knowledge that cannot be templated: the specific hospital systems competing against your practice in each city, the review count benchmarks for the local pack in each ZIP code, the search intent patterns particular to each neighborhood, and the local search landscape in each market. We only take on cities where we can answer those questions without research, because hesitation on competitive intelligence is a tell that the agency doesn't actually know your market. We are evaluating expansion into additional Florida markets beyond these — but we will not take on a new market until we can deliver that same depth there.",
  },
  {
